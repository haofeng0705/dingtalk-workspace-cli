...
func TestAonePRAdvisoryWorkflowSecurityContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"setCommentContext(pull, 'approval_required')",
		"core.setOutput('should_comment', 'false')",
		"steps.authorize.outputs.should_comment == 'true'",
		"ADVISORY_STATE: ${{ steps.authorize.outputs.advisory_state }}",
		"此前 SHA 的评测结论已失效",
		"维护者审核当前 SHA 后，可添加 `aone-ci-approved` 标签触发评测",
